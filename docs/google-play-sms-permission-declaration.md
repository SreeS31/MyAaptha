# Google Play SMS permission declaration for MyAaptha
...
2. Keep `READ_SMS` only. MyAaptha does not need `SEND_SMS`, `WRITE_SMS`, `RECEIVE_SMS`, MMS, call-log, or background SMS permissions for the current user-initiated import.
...
3. Describe the feature using text similar to:
   ...
   > MyAaptha uses READ_SMS only when a signed-in Android user chooses “Import SMS” in Money & insights. The app examines up to 200 recent messages on the device, filters for transaction-related messages, and sends only those candidate financial messages to the user’s private MyAaptha account to extract amount, direction, merchant and category. Personal conversations are excluded. The original SMS text is not retained in the financial transaction record. Users may decline permission, use manual entry, revoke permission in Android settings, and delete imported transactions.
...
- Complete **Financial features**. For the current tracker, select the closest accurate support category—normally **Other**—and do not describe MyAaptha as a broker, investment executor, bank, wallet, or licensed adviser.
