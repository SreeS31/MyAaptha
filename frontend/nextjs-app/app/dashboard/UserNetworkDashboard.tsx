...
const attachmentAccept=Array.from(circleAttachmentExtensions,extension=>`.${extension}`).join(',');
...
function CircleAttachment({post}:{post:CirclePost}) {
  ...
  const changed=()=>window.dispatchEvent(new CustomEvent('myaaptha:circle-post-changed',{detail:{circleId:post.circleId}}));
...
export default function UserNetworkDashboard({ username }: { username: string }) {
  ...
  useEffect(() => { const saved = window.localStorage.getItem('myaaptha-family-view'); if (saved === 'modern' || saved === 'heritage' || saved === 'compact') setTreeView(saved); }, []);
  ...
  useEffect(()=>{const changed=(event:Event)=>{const circleId=(event as CustomEvent<{circleId:number}>).detail?.circleId;if(circleId)void fetchCirclePosts(circleId).then(loaded=>setCirclePosts(current=>({...current,[circleId]:loaded}))).catch(error=>setMessage(errorMessage(error)));};window.addEventListener('myaaptha:circle-post-changed',changed);return()=>window.removeEventListener('myaaptha:circle-post-changed',changed);},[]);
  ...
  const selectTreeView = (view: 'modern' | 'heritage' | 'compact') => { setTreeView(view); window.localStorage.setItem('myaaptha-family-view',view); };
  ...
  const communicationMessage = (target: typeof communication) => {
    ...
    return `Hello ${target.name}, ${target.ownerName} ${action} on MyAaptha. Please log in to the application and create your own circles: ${window.location.origin}/auth`;
  };
  ...
  return <main className="container user-network-dashboard">
    <header className="network-header dashboard-compact-hero">
      <div><p className="eyebrow">MY MYAAPTHA</p><h1>Welcome, {username}</h1><p>Find people you know, define the relationship, and organize them into circles.</p></div>
    ...
    <section className="user-dashboard-metrics" aria-label="Your MyAaptha summary">
